...
View(data.frame(Dataset=c("Training","Testing"), Observations=c(dim(trainPml)[1],dim(testPml)[1]),Variables=c(dim(trainPml)[2],dim(testPml)[2])))
```
...
```{r model.sel, echo=F}
View(data.frame(rpart = cm.rpart$overall[1], rf = cm.rf$overall[1], treebag = cm.bag$overall[1]))
cm.rf
```
...
View(data.frame(Prediction=pred.test))
```
